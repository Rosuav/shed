...
int main(int argc,array(string) argv)
{
	...
	if (argc>2)
	{
		...
		if (!has_suffix(fn,".mkv"))
		{
			...
			Process.create_process(({"scp",outfn,"netbios@huix:Mythbusters/"}))->wait();
			if (!file_stat("/video/Mythbusters/"+outfn)) exit(1,"Something went wrong in transfer, scroll up for errors\n");
			rm(outfn);
		}
		else Process.create_process(({"scp",fn,"netbios@huix:Mythbusters/"+sh_quote(outfn)}))->wait();
	...
	foreach (Stdio.read_file("Mythbusters episodes")/"\n"-({""}),string line) if (mixed ex=catch
	{
		...
		if (!has_suffix(fn,".mkv"))
		{
			...
			rm(outfn);
			Process.create_process(({"ssh","huix@huix","sudo","rm","/video/Mythbusters/"+sh_quote(fn)}))->wait(); //Yes, double-escape it. SSH is weird sometimes.
		}
		else Process.create_process(({"ssh","huix@huix","sudo","mv","/video/Mythbusters/"+sh_quote(fn),"/video/Mythbusters/"+sh_quote(outfn)}))->wait();
	}) {write(line+"\n"); throw(ex);}
}
