...
# Build up a filter_complex that will split the channels out.
# Also, give each channel a descriptive title eg "[FR] front right".
filter = "channelsplit=channel_layout=" + audio["channel_layout"]
map_audio = []
for i, channel in enumerate(layouts[audio["channel_layout"]].split("+")):
	filter += f"[{channel}]"
	map_audio.extend(["-map", f"[{channel}]", "-metadata:s:a:%d" % i, f"title=[{channel}] {channel_desc[channel]}"])

# Let's do this.
args = (["ffmpeg", "-y", "-i", infile, "-filter_complex", filter]
	+ map_video + map_audio + map_subs
