...
function render(grid, posr, posc) {
	const size = Math.max(Math.min(window.innerHeight / grid.length, window.innerWidth / grid[0].length, 100), 25);
...
function improve_maze(maze, walk) {
	if (!walk.length) {
...
interval = setInterval(improve_maze, 20000 / maze.length / maze[0].length / 2, maze, []);
